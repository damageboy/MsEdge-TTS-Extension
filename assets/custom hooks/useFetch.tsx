    ...
    case "SET_LANGUAGES":
      ...
      if (action.payload.currentVoice && action.payload.currentVoice.language) {
        const countries = Object.keys(
          data[action.payload.currentVoice.language],
        ).sort();
        if (action.payload.currentVoice.country) {
          const voices =
            data[action.payload.currentVoice.language][
            action.payload.currentVoice.country
            ];
      ...
      return {
        ...state,
        data: action.payload.voices,
        languages: Object.keys(action.payload.voices).sort(),
      };
    case "SET_COUNTRIES":
      if (!state.data[action.payload.language]) return state;
      return {
        ...state,
        countries: Object.keys(state.data[action.payload.language]).sort(),
      };
    case "SET_VOICES":
      if (
        !state.data[action.payload.language] ||
        !state.data[action.payload.language][action.payload.country]
      )
        return state;
      return {
        ...state,
        voices: state.data[action.payload.language][action.payload.country],
      };
    case "SET_LOADING":
...
const formatVoices = (voices: Record<string, any>): Record<string, any> => {
  const regex =
    /Microsoft (?<name>\w+) Online.*- (?<language>\w+) \((?<country>[\w ]+)\)/;
  return voices
    .filter((v: Record<string, any>) => regex.test(v.FriendlyName))
    .map((v: Record<string, any>) => {
      const match = v.FriendlyName.match(regex);
      return {
    ...
    .reduce((acc: Record<string, any>, voice: Record<string, any>) => {
      ...
      acc[voice.language][voice.country] =
        acc[voice.language][voice.country] || {};
      acc[voice.language][voice.country][voice.name] = {
        name: voice.name,
